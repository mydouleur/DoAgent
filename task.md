...
理念：AI 只是副驾驶，把程序员的还给程序员。不写代码以外的任何事，不碰 shell。
...
1. **TUI 的 Markdown 渲染**：AI 回复目前按纯文本显示，是否做 md 解析（代码块高亮/加粗/标题/列表）——做了更耐看，但违背极简？需要权衡
2. **思考内容（reasoning）不可见**：实测中疑似 reasoning 块不显示——排查是渲染 bug、折叠默认态问题，还是 API 没返回 reasoning_content
3. **logo 位置/展示方式**：目前启动时内嵌在对话流顶部，是否改为启动画面或调整排版，再斟酌
4. **/setting 独立界面**：现在是单行命令式，考虑做成一个可交互的设置页（字段列表 + 编辑），美化交互
5. **工具调用显示样式**：折叠态目前是单行摘要，考虑统一成函数调用样式，如 `read("src/main.rs")`、`edit("a.rs", ...)`，更接近程序员直觉
