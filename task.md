...
- **TUI 中英双语 i18n**：手写零依赖方案——`crates/do/src/lang.rs` 一张静态表（`enum Key` + match 返回 (en, zh) 元组，编译器强制双语齐全，≈ C# .resx 的最小形态），54 个 key 覆盖 splash/hint/slash 反馈/三个页面/状态栏；`/setting lang zh|en` 走双层配置（lang 是身份项可 -g），默认 en；core 协议文案（工具结果/system prompt/工具描述）不动，slash 命令名不译
- **紧凑布局 + /lang**：md 渲染改三层策略——断行（`break_line`）永不制造空行，空行只由 `ensure_blank`（幂等）在重结构边界锚定：标题/代码块/分隔线前后恰一空行（文档首尾除外），段落/列表项/引用零空行；顺手修了标题样式 End 时不弹栈的历史泄漏。`/lang zh|en` 快捷切换（裸命令轮换），持久化到全局层，反馈用新语言
